...
const treatCards: TreatCard[] = [
  {
    ...
    body: "A knocked-out permanent tooth can often be saved - but only if it's reimplanted within 30-60 minutes. Keep the tooth moist (in milk, saliva, or a Save-A-Tooth kit) and call us en route.",
    eta: "30 min",
  },
  {
    badge: "Call immediately",
    ...
    body: "A dental abscess with visible swelling can become life-threatening if infection spreads. Don't wait. We'll drain it, start antibiotics, and treat the underlying tooth today.",
    eta: "1 hour",
  },
  {
    badge: "Call immediately",
      ...
      <div className="mx-auto w-full max-w-[1250px]">
        <div className="max-w-[980px]">
          <div className="flex items-center gap-3">
            ...
            <p className="text-[12px] uppercase tracking-[0.24em] text-[#43ACA9]">What We Treat</p>
          </div>
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {treatCards.map((card) => (
            <article key={card.title} className="rounded-[16px] border border-[#F0DDE1] bg-[#FFFDFD] px-5 pb-5 pt-5 sm:px-6 sm:pb-6">
              ...
              <h3 className="mt-4 font-fraunces text-[34px] leading-tight text-[#111B2D] sm:text-[40px] lg:text-[47px]">{card.title}</h3>
              <p className="mt-1 min-h-[44px] font-fraunces text-[16px] italic text-[#8A93A3] sm:text-[18px] lg:text-[20px]">{card.subtitle}</p>
              <p className="mt-4 text-[16px] leading-[1.68] text-[#49576A] sm:text-[18px] lg:text-[20px]">{card.body}</p>

              <div className="mt-5 border-t border-[#EFE6E8] pt-4">
                <div className="flex items-center justify-between">
                  <p className="text-[12px] font-semibold uppercase tracking-[0.18em] text-[#74839A]">See you in</p>
                  <p className="font-fraunces text-[30px] italic text-[#EC4252] sm:text-[34px] lg:text-[40px]">{card.eta}</p>
                </div>
              </div>
            </article>
          ))}
